...
function EosBadge({ asset }: { asset: AssetSW }) {
  const { t } = useTranslation();
...
function NewAssetModalSW({ plants, onClose }: { plants: { id: string; code: string; name: string }[]; onClose: () => void }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
...
function EditAssetModalSW({ asset, onClose }: { asset: AssetSW; onClose: () => void }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
...
function SWTab({ search }: { search: string }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th className="text-left px-4 py-3 font-medium text-gray-600">{t("assets.vendor_label")}</th>
