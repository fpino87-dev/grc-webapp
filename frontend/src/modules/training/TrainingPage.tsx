  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
        <h3 className="text-lg font-semibold mb-4">Nuovo corso</h3>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select name="source" defaultValue="interno" onChange={handleChange} className="w-full border rounded px-3 py-2 text-sm">
                {["interno","kb4","esterno"].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2 mt-6">
              ...
              <label htmlFor="mandatory" className="text-sm font-medium text-gray-700">Obbligatorio</label>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          >
            {mutation.isPending ? "Salvataggio..." : "Crea corso"}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-gray-900">Operazioni — Formazione</h2>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-gray-400">Nessun corso registrato</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-gray-600">Titolo corso</th>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600">Durata (min)</th>
                ...
                <tr key={course.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${course.mandatory ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-500"}`}>
                      {course.mandatory ? "Sì" : "No"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{course.duration_minutes ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{course.deadline ? new Date(course.deadline).toLocaleDateString("it-IT") : "—"}</td>
