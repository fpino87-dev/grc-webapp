...
const DOCUMENT_TYPES: { value: string; label: string }[] = [
  ...
  { value: "manuale", label: "Manuale ISMS" },
  { value: "contratto", label: "Contratto/NDA" },
  { value: "registro", label: "Registro" },
  { value: "altro", label: "Altro" },
];

const NORMATIVE_ROLE_LABELS: Record<string, string> = {
  ciso: "CISO",
...
const ALL_NORMATIVE_ROLES = Object.entries(NORMATIVE_ROLE_LABELS).map(([value, label]) => ({
  value,
  label,
}));
...
function RoleMultiSelect({
  ...
  return (
    <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto border rounded px-2 py-1 bg-gray-50">
      {ALL_NORMATIVE_ROLES.map((r) => (
        <label key={r.value} className="flex items-center gap-1 text-xs text-gray-700">
          <input
            type="checkbox"
...
export function DocumentWorkflowSection({ embedded }: { embedded?: boolean }) {
  ...
  function scopeLabel(p: DocumentWorkflowPolicy): string {
    if (p.scope_type === "org") return "Organizzazione";
    if (p.scope_type === "bu") return "Business Unit (ID)";
    if (p.scope_type === "plant") {
      const plant = plants.find((pl) => pl.id === p.scope_id);
      if (plant) return `Sito: ${plant.code} — ${plant.name}`;
      return "Sito (ID)";
    }
    return p.scope_type;
  }
  ...
  return (
    <div className={embedded ? "space-y-5" : "p-6 max-w-5xl mx-auto space-y-5"}>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        {isLoading ? (
          <div className="p-6 text-sm text-gray-400">Caricamento...</div>
        ) : !policies.length ? (
          <div className="p-6 text-sm text-gray-400 italic">
            Nessuna policy configurata. Creane una per tipo documento.
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-4 py-2 text-left font-medium text-gray-600">Tipo documento</th>
                <th className="px-4 py-2 text-left font-medium text-gray-600">Scope</th>
                <th className="px-4 py-2 text-left font-medium text-gray-600">Chi può inviare</th>
                <th className="px-4 py-2 text-left font-medium text-gray-600">Chi revisiona</th>
                <th className="px-4 py-2 text-left font-medium text-gray-600">Chi approva</th>
              ...
              {policies.map((p) => (
                <tr key={p.id}>
                  <td className="px-4 py-2 text-gray-800">
                    {DOCUMENT_TYPES.find((d) => d.value === p.document_type)?.label || p.document_type}
                  </td>
                  <td className="px-4 py-2 text-xs text-gray-700">{scopeLabel(p)}</td>
                  <td className="px-4 py-2 text-xs text-gray-700">
                    {p.submit_roles?.length
                      ? p.submit_roles.map((r) => NORMATIVE_ROLE_LABELS[r] ?? r).join(", ")
                      : "—"}
                  </td>
                  <td className="px-4 py-2 text-xs text-gray-700">
                    {p.review_roles?.length
                      ? p.review_roles.map((r) => NORMATIVE_ROLE_LABELS[r] ?? r).join(", ")
                      : "—"}
                  </td>
                  <td className="px-4 py-2 text-xs text-gray-700">
                    {p.approve_roles?.length
                      ? p.approve_roles.map((r) => NORMATIVE_ROLE_LABELS[r] ?? r).join(", ")
                      : "—"}
                  </td>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">
                {editing.id ? "Modifica policy" : "Nuova policy"}
              </h3>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <p className="text-xs font-semibold text-gray-700 mb-1">
                  Chi può creare / inviare in revisione
                </p>
                <RoleMultiSelect
              ...
              <div>
                <p className="text-xs font-semibold text-gray-700 mb-1">Chi revisiona</p>
                <RoleMultiSelect
              ...
              <div>
                <p className="text-xs font-semibold text-gray-700 mb-1">Chi approva</p>
                <RoleMultiSelect
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">Elimina policy</h3>
            <p className="text-sm text-gray-600">
              Sei sicuro di voler eliminare la policy per{" "}
              <strong>
                {DOCUMENT_TYPES.find((d) => d.value === confirmDelete.document_type)?.label ||
