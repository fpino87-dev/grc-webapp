          ...
          {step === 2 && state.process && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">{t("risk.cause_label")}</label>
                    <textarea
                      className="w-full border rounded px-3 py-1.5 text-sm resize-none"
                      rows={2}
                      placeholder={t("risk.cause_placeholder")}
                      value={riskForm.cause as string ?? ""}
                      onChange={e => setRiskForm(f => ({ ...f, cause: e.target.value }))}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">{t("risk.consequence_label")}</label>
                    <textarea
                      className="w-full border rounded px-3 py-1.5 text-sm resize-none"
                      rows={2}
                      placeholder={t("risk.consequence_placeholder")}
                      value={riskForm.consequence as string ?? ""}
                      onChange={e => setRiskForm(f => ({ ...f, consequence: e.target.value }))}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
