...
backend (Django + DRF)
    │
    ├── apps/          une application Django par module (M00–M21 + OSINT)
    ...
    │
    └── Celery Worker  tâches asynchrones : notifications, jobs audit trail
        Celery Beat    planificateur récurrent : échéances et rappels, sauvegarde nocturne, KPI, OSINT
```
...
│   ├── training/            # M15 — Formation fondée sur les preuves
│   ├── bcp/                 # M16 — PCA
...
│   ├── cockpit/             # M21 — Centre opérationnel (insights + advisors)
│   ├── osint/               # OSINT Monitor (transversal, org-wide)
│   └── compliance_schedule/ # Échéances configurables (règles M08)
└── frameworks/
    ├── iso27001.json
    ├── nis2.json
    ├── tisax_l2.json
...
│   │   ├── Sidebar.tsx        # Navigation latérale avec entrées pour M00–M21 + OSINT
...
├── modules/                   # Un dossier par module (M00–M21 + OSINT)
...
- Niveaux de classification L1/L2/L3 (référence 5/3/1 ans) ; log permanent/immuable, aucune suppression automatique
- Vérification : `python manage.py verify_audit_trail_integrity`

### ControlInstance

- Champ `applicability` pour la SOA ISO 27001
- `calc_maturity_level` (propriété) pour VDA ISA (échelle 0-5)
- `implementation_description` — « Implementation description » VDA ISA (comment l'exigence est mise en œuvre sur le site) ; écrite uniquement via `POST /set-implementation/` → `set_implementation_description()` (audit `control.implementation_description_set`, payload avec la seule longueur), exportée dans le VDA ISA ; advisor M21 `controls.tisax_missing_implementation` pour ML ≥ 3 sans description
- `implementation_interview` — entretien guidé VDA ISA « façon auditeur » (`services/vda_interview.py`, `VdaInterviewPermission` : uniquement les rôles qui évaluent les contrôles) : `GET|POST /vda-interview/`, `POST /vda-interview/review/`, `POST /vda-interview/draft/`. Exigences extraites de la description EN du référentiel (sections must / should / protection élevée / très élevée ; pour un VH aussi celles du contrôle de base étendu), `req_id` = hash du texte. L'IA les regroupe en 2 à 4 thèmes (question, intention de l'auditeur, éléments à mentionner, exemple à espaces réservés) mis en cache dans `Control.translations[lang]["interview_topics"]`, liés à l'ensemble des exigences (texte normatif, sans sanitize) ; au premier enregistrement, thèmes et langue deviennent un instantané dans l'état. Vérification auditeur max 2 tours (`MAX_REVIEW_ROUNDS`) : couverture par exigence, ≤ 3 approfondissements, preuves, maturité soutenue ; `reset_reviews` la recommence. Brouillon (tâche IA `vda_interview`, Sanitizer) à partir de tout l'échange avec `[TO BE COMPLETED]` pour les exigences obligatoires `missing` lors de la dernière vérification ; il n'est pas enregistré, et l'enregistrement avec `ai_interaction_id` confirme l'interaction IA (`ai_assisted` dans l'audit)
- `needs_revaluation` pour la gestion du changement (M04)
- `na_justification` — texte libre sauvegardé par `evaluate_control()` quand status=`na` ; exposé dans l'API détail et utilisé par le moteur d'export (VDA et SOA)

**Mapping `calc_maturity_level` :**

| Statut | ML | Notes |
|--------|----|-------|
| `non_valutato` | 0 | — |
| `gap` | 1 | — |
| `na` | 0 | Exclu de la moyenne ML dans le VDA ISA |
| `parziale` | 2 | — |
| `compliant` | 3 / 4 | 4 si ≥ 2 preuves valides ; 5 uniquement par override manuel (`set-maturity`) |
...
class ControlInstance(BaseModel):
    ...
    na_justification = models.TextField(blank=True)       # justification N/A — utilisée par VDA et SOA
...
### Service `propagate_control()`

Défini dans `apps/controls/services.py`. Propage le statut d'un `ControlInstance` aux contrôles mappés en respectant le type de relation et la direction.

```python
propagate_control(instance, user, cross_plant=False) -> dict
# Retourne : {"propagated_to": int, "skipped_no_instance": int}
```

**Règles de propagation :**

| Relation (`ControlMapping.relationship`) | Direction de propagation |
|------------------------------------------|--------------------------|
| `equivalente` | Bidirectionnelle (A→B et B→A) |
| `covers` | Uniquement source→cible |
| `parziale`, `correlato`, `extends` | Non propagé |

- Seuls les statuts `compliant` et `na` sont propagables.
- Chaque instance mise à jour génère un enregistrement d'audit (`control.propagated`).
- Pour `na` : copie `na_justification` avec le préfixe "Propagato da {external_id}".
- `cross_plant=True` : propage à tous les plants disposant d'une instance active du contrôle cible.

**Endpoint REST :**
```
POST /api/controls/instances/{id}/propagate/
Body (optionnel) : { "cross_plant": true }
Réponse : { "propagated_to": 3, "skipped_no_instance": 0 }
```
...
| `reporting/compliance-overview/` | GET | Conformité : synthèse par référentiel (%, statuts, tendance) et comparaison sites × référentiels |
| `reporting/compliance-domains/` | GET | Conformité : contrôles ouverts par domaine d'un référentiel (`&export=csv` pour les contrôles ouverts) |
| `notifications/` | GET | Notifications M19 |
| `cockpit/insights/` | GET | Insights agrégés + posture M21 |
| `cockpit/insights/{fingerprint}/{action}/` | POST | Action insight (snooze\|accept\|reopen) |
| `osint/entities/` | GET, POST | Entités surveillées OSINT |
| `osint/alerts/` | GET | Alertes d'exposition OSINT |
| `osint/dashboard/` | GET | Tableau de bord d'exposition OSINT |
...
- L1 (sécurité), L2 (conformité), L3 (opérationnel) : niveaux de **classification** des événements (référence de conservation : 5/3/1 ans)
- **Aucune tâche de suppression** : l'audit log est append-only/immuable. Le trigger PostgreSQL rejette UPDATE et DELETE, il n'existe donc pas de rétention par suppression (règle #4). La conservation est de fait permanente ; L1/L2/L3 ne sont que des niveaux de classification.
...
# À lancer à la demande ou via cron (aucune tâche beat planifiée par défaut)
# Sort avec un code non nul si la chaîne est rompue
```
...
```
apps/ai_engine/
├── sanitizer.py                            # anonymisation des PII avant le cloud
├── router.py                               # routage local/cloud par fonction, confirm/ignore
├── tasks_ai.py                             # fonctions IA : classification des incidents, explication des contrôles, actions sur les écarts, brouillon RCA
├── agent_orchestrator.py + agent_tools.py  # assistant GRC : écarts du site et explications
├── catalog.py                              # catalogue des modèles des fournisseurs
├── circuit_breaker.py                      # disjoncteur vers les fournisseurs
├── models.py                               # AiInteractionLog
├── tasks.py                                # rétention d'AiInteractionLog (nettoyage mensuel)
└── tests/
```
...
def suggest_severity(incident, user) -> dict | None:
    try:
        result = route(
            task_type="incident_classify",   # local ou cloud selon le routage dans Paramètres → AI Engine
            prompt=f"...{incident.description}",
            user=user,
            entity_id=incident.pk,
            module_source="M09",
            sanitize=True,                   # obligatoire avec du texte libre (règle #9)
            plant_ids=[incident.plant_id],
        )
    except (AiNotConfigured, LlmUnavailable):
        return None                          # IA non configurée ou injoignable : aucune suggestion
    # result = {"text", "provider", "model", "interaction_id", ...}
    # Proposé à l'utilisateur, jamais appliqué seul : confirm_output() / ignore_output()
    return result
```
...
### Formation (M15) : aucune intégration externe

M15 ne s'intègre à aucune plateforme d'e-learning ou de simulation de phishing (l'intégration KnowBe4 a été supprimée) : la plateforme **pilote** la formation, elle ne la dispense pas. Les données sont uniquement des chiffres par groupe : aucun registre du personnel.

- **Modèle** (`apps/training/models.py`) : `TrainingCourse` (type, public, `validity_months`, `plants` = périmètre : vide = cours d'organisation, sinon ces sites uniquement ; `controls` déprécié), `TrainingEvidenceControl` (paramètre unique : par `audience_kind`, les `Control` qu'une session prouve ; valeurs initiales via `load_training_evidence_controls`, la migration `training.0006` y copie les anciens `controls` des cours actifs), `TrainingAudience` (groupe cible par site avec `headcount`), `TrainingPlan` (par site ou de l'organisation, avec un `document` M07 pour l'approbation), `TrainingPlanItem`, `TrainingSession` (session avec chiffres, `evidence` et indicateur `legacy`).
- **Services** (`apps/training/services.py`) : `register_session` valide le fichier (`validate_uploaded_file`), crée l'`Evidence` et la lie aux `ControlInstance` du site (`link_evidence_to_controls`) ; `remind_plan_items` ouvre les tâches pour le rôle `compliance_officer` (source_module `M15`) ; indicateurs partagés par KPI, Reporting et Cockpit (`training_coverage`, `plan_progress`, `latest_phishing`, `expiring_training_evidence`).
- Périmètre et liaison : `create_course`/`update_course`/`delete_course` exigent un périmètre d'organisation pour les cours sans site et la gestion de tous leurs sites pour les autres ; `course_applies_to` limite les éléments du plan (plan d'organisation → cours d'organisation uniquement) et les sessions aux cours valables pour le site. `link_evidence_to_controls` utilise `evidence_controls_for(audience_kind)` filtré sur `get_active_frameworks(plant)` : les contrôles des référentiels non appliqués au site sont ignorés, non signalés. API `/api/v1/training/evidence-controls/` (GET/POST/DELETE ; écriture réservée au périmètre d'organisation).
- **Permissions** : écriture pour `super_admin`/`compliance_officer`/`plant_manager` dans leur périmètre ou pour une nomination CISO active dans Governance (`can_manage_training`) ; lecture aussi pour les auditeurs. `GET /api/v1/training/courses/capabilities/` indique à l'interface quoi afficher.
- **Tâche Celery** : `remind-training-plan-items`, chaque jour à 08:10.
- **Rôles critiques et organe de direction** : pour les cours dont `audience_kind` n'est pas `generale` (phishing exclu, `TrainingCourse.is_named`), la session enregistre des `TrainingParticipant` (FK `user` et/ou `committee_member`, `roles` = instantané des nominations actives). `participant_options(plant, day)` liste les titulaires de `RoleAssignment` actives couvrant le site et les `CommitteeMember` en fonction des organes du site ou sans site ; `register_session` accepte `participant_users`/`participant_members`, les valide sur ces options et fusionne les personnes à la fois membres et titulaires. Si le cours a `competency`/`competency_level`, `_apply_competency` met à jour la `UserCompetency` (preuve, `valid_until`, `verified_by`) sans abaisser le niveau ni remplacer une preuve plus récente, en conservant l'état précédent dans `competency_before` ; à la suppression de la session, `_revert_competencies` le restaure (en remontant les sessions déjà supprimées) ou supprime logiquement la compétence née de la session. `UserCompetency` a désormais une contrainte d'unicité conditionnelle (`uniq_user_competency_alive`, lignes non supprimées uniquement). `board_training(plant)` alimente le KPI interne `board_training_valid` (membres en fonction des `SecurityCommittee` de type `cda` ayant une session valide d'un cours `organo_gestione`), la section Reporting (effectifs seulement), le `board_training.csv` du pack d'audit et `GET /api/v1/training/sessions/board-status/?plant=`. Autres endpoints : `GET sessions/participant-options/?plant=&held_on=` (réservé à ceux qui gèrent la formation du site) et `GET courses/competency-options/`. L'audit trail ne contient que des id et `participants` (un nombre).
- **Données historiques** : les inscriptions et résultats de phishing individuels (`TrainingEnrollment`, `PhishingSimulation`) sont agrégés par la migration `training.0004` en sessions `legacy` ne contenant que des comptages ; la migration `training.0007`, dans le même `migrate`, supprime ensuite les tables individuelles et les champs obsolètes du cours (`framework_refs`, `controls`).
...
| `check_training_migration_readiness` | Aperçu en lecture seule de la migration des données de formation (`training.0004`) | Avant le `migrate` qui l'applique |
| `load_training_evidence_controls` | Ajoute les contrôles par défaut prouvés par les sessions de formation (ACN PR.AT-01/02, ISO A.6.3, NIS2 ART21-7.1, TISAX ISA-2.1.3), seulement s'ils existent ; idempotent, ne supprime rien | Après `load_frameworks`, au premier déploiement de la formation fondée sur les preuves |
...
| `DATABASE_URL` | string | postgresql://grc:REPLACE_DB_PASSWORD@db:5432/grc_dev | URL PostgreSQL | Oui |
...
1. Vérifier dans **Paramètres → AI Engine** qu'une configuration active existe et que la fonction est routée vers le bon fournisseur (routage par fonction)
