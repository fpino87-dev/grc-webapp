...
**Mevcut PDCA'lar, kapalı olanlar dahil** — bir bulgu, aynı tesisin herhangi bir PDCA'sına, **zaten kapalı veya arşivlenmiş olsa bile** bağlanabilir: bu, bulgu kaydedilmeden önce yapılmış bir düzeltici faaliyeti (ör. denetim raporundan elle açılmış bir PDCA) veya geçmişin kurtarılmasını kapsar. Bulgu açıksa ve PDCA kapalıysa aşağıda açıklanan PDCA kapanış kuralları geçerlidir (etkili sonuçta bulgu hemen kapatılır); bulgu zaten kapalıysa hiçbir durum değişmez (geriye dönük bağlantı, denetim izinde bu şekilde kaydedilir). PDCA menüsünde **🔗 Bulgular**, denetimin açık ve kapalı tüm bulgularını gösterir.

**PDCA'yı değiştirme** — bulgunun zaten bir PDCA'sı varsa (NC'ler için otomatik açılan), bir neden belirtilerek doğrusuyla değiştirilir: bulgudan **PDCA'yı değiştir** ile veya PDCA menüsünden bulguyu seçerek (bağlı olduğu döngüyü gösterir). Değiştirilen PDCA yalnızca o bulgu için otomatik açılmış ve hiç üzerinde çalışılmamışsa arşivlenir; aksi halde olduğu gibi kalır, yalnızca bağlantısı kaldırılır. Kapalı bir bulgu için yeni PDCA açılmaz: mevcut bir PDCA'ya bağlanır.
...
Bulgunun bağlı bir PDCA'sı varsa, yalnızca düzeltici faaliyet tamamlandığında kapatılır: PDCA **kapalı** (veya arşivlenmiş) ya da yalnızca bu bulguyu kapsıyorsa **ACT** aşamasındaysa; bu durumda PDCA bulguyla birlikte kapatılır ve kapanış notları standartlaştırması olur. Aksi halde sistem PDCA aşamasını gösterir ve hiçbir şey kaydetmez. **PDCA'dan kapanış** — PDCA zaten kanıtı (DO aşaması), etkinlik doğrulamasını (CHECK) ve standartlaştırmayı (ACT) içerir; bu nedenle kapatıldığında bağlı bulgular CHECK sonucunu izler: **etkili** → DO aşamasının kanıtı ve kapanış notu olarak ACT açıklamasıyla PDCA ile birlikte kapatılır (NC için DO kanıtı gerekir, aksi halde "yanıtta" kalır); **kısmen etkili** → "yanıtta" durumuna geçer ve kapanışa Bulgular sekmesinden siz karar verirsiniz; **etkisiz** → yeni "[Riciclo]" döngüsüne taşınır ve açık kalır. Ek bir alınan ders oluşturulmaz: PDCA'nınki yeterlidir. PDCA zaten kapalıyken "yanıtta" kalmış bir bulgu (ör. bu kuraldan önce bağlanmış ya da yeterli gördüğünüz kısmi bir sonuçla) **PDCA ile kapat** düğmesiyle, DO kanıtı ve ACT açıklaması yeniden girilmeden kapatılır.
...
**Bulguyu düzeltme** — bulgu kartındaki **✎ Düzenle** ile başlığı ve açıklamayı düzeltin (ör. bir yazım hatası). Ortak bulguda düzeltme tüm tesisler için geçerlidir. Tür, tarihler ve durum düzenlenemez: PDCA ve kapatma üzerinden değişir. Arşivlenmiş denetimlerin bulguları artık düzenlenemez.

**Takip edilmeyen bulgu** — uygulamamaya karar verdiğiniz bir gözlem veya fırsat için bulgu kartındaki **Takip etme** düğmesini kullanın. Gerekçeyi yazın (zorunlu, en az 20 karakter) ve isterseniz bir kanıt ekleyin: bir dosya (ör. kararın tutanağı) veya mevcut bir kanıt. Bulgu **Takip edilmedi** durumuna geçer: artık açık sayılmaz, ancak denetim paketinde ve reddedilen bulguları gerekçeleriyle listeleyen yönetim gözden geçirmesinde kapalı bulgulardan ayrı kalır. Bağlı ve hâlâ açık olan PDCA aynı gerekçeyle arşivlenir; ortak bulguda karar tüm tesisler için geçerlidir. **Yeniden aç** ile bulgu tekrar açılır ve yeni bir PDCA açılır veya bağlanır. Uygunsuzluklar takip edilmeden bırakılamaz: PDCA'yı izler. Aynı sonuç PDCA modülünde 📦 **Arşivle** ile de elde edilir: pencere gerekçeyi ve isteğe bağlı kanıtı ister, bağlı ve hâlâ açık gözlemler ile fırsatlar «Takip edilmedi» durumuna geçer; bağlı uygunsuzluklar açık kalır ve pencere bunu belirtir. Bulgusu olmayan bir PDCA'da kanıt döngüde kalır ve dosyasında görünür.
