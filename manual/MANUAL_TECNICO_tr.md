...
- `implementation_interview` — VDA ISA "denetçi tarzı" rehberli görüşme (`services/vda_interview.py`, `VdaInterviewPermission`: yalnızca kontrolleri değerlendiren roller): `GET|POST /vda-interview/`, `POST /vda-interview/review/`, `POST /vda-interview/draft/`. Gereksinimler çerçevenin EN açıklamasından çıkarılır (must / should / yüksek / çok yüksek koruma bölümleri; VH için genişletilen temel kontrolün gereksinimleri de), `req_id` = metnin hash'i. YZ bunları 2-4 konuya gruplar (soru, denetçi amacı, bahsedilecekler, yer tutuculu örnek); gereksinim kümesine bağlı olarak `Control.translations[lang]["interview_topics"]` içinde önbelleğe alınır (normatif metin, sanitize yok); ilk kayıtta konular ve dil durumda anlık görüntü olur. Denetçi doğrulaması en fazla 2 tur (`MAX_REVIEW_ROUNDS`): gereksinim başına kapsam, ≤ 3 ek soru, kanıtlar, desteklenen olgunluk; `reset_reviews` yeniden başlatır. Taslak (YZ görevi `vda_interview`, Sanitizer) tüm görüşmeden, son doğrulamada `missing` olan zorunlu gereksinimler için `[TO BE COMPLETED]` ile; kaydedilmez ve `ai_interaction_id` ile kaydetmek YZ etkileşimini onaylar (denetimde `ai_assisted`)
...
class ControlInstance(BaseModel):
    ...
    na_justification = models.TextField(blank=True)       # N/A gerekçesi — VDA ve SOA tarafından kullanılır
...
### `propagate_control()` servisi

`apps/controls/services.py` dosyasında tanımlanmıştır. Bir `ControlInstance`'ın durumunu, ilişki türü ve yönüne göre eşlenmiş kontrollere yayar.

```python
propagate_control(instance, user, cross_plant=False) -> dict
# Döndürür: {"propagated_to": int, "skipped_no_instance": int}
```

**Yayma kuralları:**

| İlişki (`ControlMapping.relationship`) | Yayma yönü |
|----------------------------------------|------------|
| `equivalente` | Çift yönlü (A→B ve B→A) |
| `covers` | Yalnızca kaynak→hedef |
| `parziale`, `correlato`, `extends` | Yayılmaz |

- Yalnızca `compliant` ve `na` durumları yayılabilir.
- Güncellenen her örnek bir denetim izi kaydı oluşturur (`control.propagated`).
- `na` için: `na_justification` "Propagato da {external_id}" önekiyle kopyalanır.
- `cross_plant=True`: hedef kontrolün aktif örneğine sahip tüm tesislere yayar.

**REST uç noktası:**
```
POST /api/controls/instances/{id}/propagate/
Gövde (isteğe bağlı): { "cross_plant": true }
Yanıt: { "propagated_to": 3, "skipped_no_instance": 0 }
```
...
- **Model** (`apps/training/models.py`): `TrainingCourse` (tür, hedef kitle, `validity_months`, `plants` = kapsam: boş = kuruluş kursu, aksi halde yalnızca o tesisler; `controls` kullanımdan kalktı), `TrainingEvidenceControl` (tek ayar: her `audience_kind` için bir oturumun kanıtladığı `Control` kayıtları; başlangıç değerleri `load_training_evidence_controls` ile, `training.0006` taşıması etkin kursların eski `controls` alanlarını buraya kopyalar), `TrainingAudience` (`headcount` içeren tesis hedef grubu), `TrainingPlan` (tesis veya kuruluş geneli, onay için M07 `document`), `TrainingPlanItem`, `TrainingSession` (sayılar, `evidence` ve `legacy` işaretiyle oturum).
...
- Kapsam ve bağlama: `create_course`/`update_course`/`delete_course`, tesissiz kurslar için kuruluş kapsamı, diğerleri için tüm tesislerinin yönetimini gerektirir; `course_applies_to`, plan kalemlerini (kuruluş planı → yalnızca kuruluş kursları) ve oturumları tesis için geçerli kurslarla sınırlar. `link_evidence_to_controls`, `get_active_frameworks(plant)` ile filtrelenmiş `evidence_controls_for(audience_kind)` kullanır: tesiste uygulanmayan çerçevelerin kontrolleri bildirilmeden atlanır. API `/api/v1/training/evidence-controls/` (GET/POST/DELETE; yazma yalnızca kuruluş kapsamıyla).
...
| `load_training_evidence_controls` | Eğitim oturumlarının kanıtladığı varsayılan kontrolleri ekler (ACN PR.AT-01/02, ISO A.6.3, NIS2 ART21-7.1, TISAX ISA-2.1.3), yalnızca mevcutsa; idempotent, hiçbir şeyi silmez | `load_frameworks` sonrasında, kanıta dayalı eğitimin ilk dağıtımında |
