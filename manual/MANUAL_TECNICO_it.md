...
backend (Django + DRF)
    │
    ├── apps/          un'app Django per modulo (M00–M21 + OSINT)
...
│   ├── training/            # M15 — Formazione a evidenze
│   ├── bcp/                 # M16 — BCP
...
│   ├── cockpit/             # M21 — Centro Operativo (insight + advisor)
│   ├── osint/               # OSINT Monitor (trasversale, org-wide)
│   └── compliance_schedule/ # Scadenze configurabili (regole M08)
└── frameworks/
    ├── iso27001.json
    ├── nis2.json
    ├── tisax_l2.json
...
```
frontend/src/
├── App.tsx                    # Router — moduli, scadenzario, impostazioni
...
│   └── endpoints/             # client API TypeScript (~24 file)
├── components/
│   ├── layout/
│   │   ├── Shell.tsx          # Layout principale con sidebar
│   │   ├── Sidebar.tsx        # Navigazione laterale con voci per M00–M21 + OSINT
...
├── modules/                   # Una cartella per modulo (M00–M21 + OSINT)
...
- Livelli L1/L2/L3 di classificazione (riferimento 5/3/1 anni); log permanente/immutabile, nessuna cancellazione automatica
...
- `implementation_interview` — intervista guidata VDA ISA "da auditor" (`services/vda_interview.py`, `VdaInterviewPermission`: solo ruoli che valutano i controlli): `GET|POST /vda-interview/`, `POST /vda-interview/review/`, `POST /vda-interview/draft/`. Requisiti estratti dalla descrizione EN del framework (sezioni must / should / alta / molto alta protezione; per un VH anche quelli del base esteso), `req_id` = hash del testo. L'IA li raggruppa in 2-4 temi (domanda, intento dell'auditor, cosa citare, esempio a segnaposto) in cache su `Control.translations[lang]["interview_topics"]` legata all'insieme dei requisiti (testo normativo, niente sanitize); al primo salvataggio i temi e la lingua diventano una fotografia nello stato. Verifica auditor max 2 giri (`MAX_REVIEW_ROUNDS`): copertura per requisito, ≤ 3 approfondimenti, evidenze, maturità sostenuta; `reset_reviews` la ricomincia. Bozza (task IA `vda_interview`, Sanitizer) dall'intera conversazione con `[TO BE COMPLETED]` per i requisiti obbligatori `missing` nell'ultima verifica; non viene salvata e il salvataggio con `ai_interaction_id` conferma l'interazione IA (`ai_assisted` nell'audit)
...
| `non_valutato` | 0 | — |
| `gap` | 1 | — |
| `na` | 0 | Escluso dalla media ML nel VDA ISA |
| `parziale` | 2 | — |
| `compliant` | 3 / 4 | 4 se ha ≥ 2 evidenze valide; il 5 solo con override manuale (`set-maturity`) |
...
class ControlInstance(BaseModel):
    ...
    na_justification = models.TextField(blank=True)       # giustificazione N/A — usata da VDA e SOA
...
### Servizio `propagate_control()`

Definito in `apps/controls/services.py`. Propaga lo stato di un `ControlInstance` ai controlli mappati rispettando tipo di relazione e direzione.
...
| Relazione (`ControlMapping.relationship`) | Direzione propagazione |
|------------------------------------------|------------------------|
| `equivalente` | Bidirezionale (A→B e B→A) |
| `covers` | Solo source→target (A copre B; non viceversa) |
| `parziale`, `correlato`, `extends` | Non propagato |

- Solo stati `compliant` e `na` sono propagabili.
- Ogni istanza aggiornata genera audit trail `control.propagated`.
- Per `na`: copia `na_justification` con prefisso "Propagato da {external_id}".
- `cross_plant=True`: propaga a tutti i plant con un'istanza attiva del controllo target (non solo al plant sorgente).
- Non esegue validazione evidenze (il dato è già stato validato sul sorgente).

**Endpoint REST:**
```
POST /api/controls/instances/{id}/propagate/
Body (opzionale): { "cross_plant": true }
Response: { "propagated_to": 3, "skipped_no_instance": 0 }
```
...
| `reporting/compliance-overview/` | GET | Compliance: sintesi per framework (%, stati, andamento) e confronto siti × framework |
| `reporting/compliance-domains/` | GET | Compliance: controlli aperti per dominio di un framework (`&export=csv` per i controlli aperti) |
| `notifications/` | GET | Notifiche M19 |
| `cockpit/insights/` | GET | Insight aggregati + posture M21 |
| `cockpit/insights/{fingerprint}/{action}/` | POST | Azione insight (snooze\|accept\|reopen) |
| `osint/entities/` | GET, POST | Entità monitorate OSINT |
| `osint/alerts/` | GET | Alert esposizione OSINT |
| `osint/dashboard/` | GET | Dashboard esposizione OSINT |
...
- L1 (sicurezza), L2 (compliance), L3 (operativo): livelli di **classificazione** degli eventi (riferimento di conservazione: 5/3/1 anni)
- **Nessun task di cancellazione**: l'audit log è append-only/immutabile. Il trigger PostgreSQL rifiuta UPDATE e DELETE, quindi non esiste retention-by-deletion (regola #4). La conservazione è di fatto permanente; L1/L2/L3 sono solo livelli di classificazione.
...
# Eseguire su richiesta o via cron (nessun task beat schedulato di default)
# Esce con codice diverso da zero se la catena è rotta
```
...
def suggest_severity(incident, user) -> dict | None:
    try:
        result = route(
            task_type="incident_classify",   # locale o cloud secondo il routing in Impostazioni → AI Engine
            prompt=f"...{incident.description}",
            user=user,
            entity_id=incident.pk,
            module_source="M09",
            sanitize=True,                   # obbligatorio con testo libero (regola #9)
            plant_ids=[incident.plant_id],
        )
    except (AiNotConfigured, LlmUnavailable):
        return None                          # IA non configurata o non raggiungibile: nessun suggerimento
    # result = {"text", "provider", "model", "interaction_id", ...}
    # Proposto all'utente, mai applicato da solo: confirm_output() / ignore_output()
    return result
```
...
### Formazione (M15): nessuna integrazione esterna

M15 non si integra con piattaforme di e-learning o di simulazione phishing (l'integrazione KnowBe4 è stata rimossa): la piattaforma **governa** la formazione, non la eroga. I dati sono solo conteggi per gruppo: nessuna anagrafica dei dipendenti.

- **Modello** (`apps/training/models.py`): `TrainingCourse` (tipo, destinatari, `validity_months`, `plants` = ambito: vuoto = corso di organizzazione, altrimenti solo quei siti; `controls` deprecato), `TrainingEvidenceControl` (impostazione unica: per `audience_kind` i `Control` che un'erogazione prova; valori iniziali da `load_training_evidence_controls`, la migrazione `training.0006` vi copia i vecchi `controls` dei corsi attivi), `TrainingAudience` (gruppo destinatari per sito con `headcount`), `TrainingPlan` (per sito o di organizzazione, con `document` M07 per l'approvazione), `TrainingPlanItem`, `TrainingSession` (erogazione con conteggi, `evidence` e flag `legacy`).
- **Servizi** (`apps/training/services.py`): `register_session` valida il file (`validate_uploaded_file`), crea l'`Evidence` e la collega alle `ControlInstance` del sito (`link_evidence_to_controls`); `remind_plan_items` apre i task al ruolo `compliance_officer` (source_module `M15`); indicatori condivisi da KPI, Reporting e Cockpit (`training_coverage`, `plan_progress`, `latest_phishing`, `expiring_training_evidence`).
- Ambito e collegamento: `create_course`/`update_course`/`delete_course` richiedono il perimetro di organizzazione per i corsi senza siti e la gestione di tutti i siti per gli altri; `course_applies_to` limita voci del piano (piano di organizzazione → solo corsi di organizzazione) ed erogazioni ai corsi validi per il sito. `link_evidence_to_controls` usa `evidence_controls_for(audience_kind)` filtrato su `get_active_frameworks(plant)`: i controlli di framework non applicati al sito vengono ignorati, non segnalati. API `/api/v1/training/evidence-controls/` (GET/POST/DELETE; scrittura solo con perimetro di organizzazione).
- **Permessi**: scrittura ai ruoli `super_admin`/`compliance_officer`/`plant_manager` nel loro perimetro o alla nomina CISO attiva in Governance (`can_manage_training`); lettura anche agli auditor. `GET /api/v1/training/courses/capabilities/` dice all'interfaccia cosa mostrare.
...
La suite pytest (`backend/pytest.ini`, `--cov=apps --cov=core --cov-fail-under=70`) copre la maggior parte delle app; la copertura globale è monitorata in CI (vedi `CLAUDE.md` per statistiche aggiornate).
...
| `check_training_migration_readiness` | Anteprima in sola lettura della migrazione dei dati di formazione (`training.0004`) | Prima del `migrate` che la applica |
| `load_training_evidence_controls` | Aggiunge i controlli predefiniti provati dalle erogazioni della formazione (ACN PR.AT-01/02, ISO A.6.3, NIS2 ART21-7.1, TISAX ISA-2.1.3), solo se presenti; idempotente, non toglie nulla | Dopo `load_frameworks`, al primo deploy della formazione a evidenze |
...
| `DATABASE_URL` | string | postgresql://grc:REPLACE_DB_PASSWORD@db:5432/grc_dev | PostgreSQL URL | Sì |
...
1. Verifica in **Impostazioni → AI Engine** che esista una configurazione attiva e che la funzione sia instradata al provider giusto (routing per funzione)
