...
│   ├── training/            # M15 — Evidence-based training
│   ├── bcp/                 # M16 — BCP
...
- L1/L2/L3 classification levels (5/3/1 year reference); permanent/immutable log, no automatic deletion
- Verification: `python manage.py verify_audit_trail_integrity`

### ControlInstance

- `applicability` field for ISO 27001 SOA
- `calc_maturity_level` (property) for VDA ISA (scale 0–5)
- `implementation_description` — VDA ISA "Implementation description" (how the requirement is implemented at the site); written only via `POST /set-implementation/` → `set_implementation_description()` (audit `control.implementation_description_set`, payload with length only), exported in the VDA ISA; M21 advisor `controls.tisax_missing_implementation` for ML ≥ 3 without description
- `implementation_interview` — VDA ISA "auditor-style" guided interview (`services/vda_interview.py`, `VdaInterviewPermission`: only roles that evaluate controls): `GET|POST /vda-interview/`, `POST /vda-interview/review/`, `POST /vda-interview/draft/`. Requirements extracted from the framework's EN description (must / should / high / very high protection sections; for a VH also those of the extended base), `req_id` = hash of the text. The AI groups them into 2-4 topics (question, auditor intent, what to mention, placeholder example) cached in `Control.translations[lang]["interview_topics"]`, tied to the requirement set (normative text, no sanitize); on first save topics and language become a snapshot in the state. Auditor review max 2 rounds (`MAX_REVIEW_ROUNDS`): coverage per requirement, ≤ 3 follow-ups, evidence, supported maturity; `reset_reviews` restarts it. Draft (AI task `vda_interview`, Sanitizer) from the whole conversation with `[TO BE COMPLETED]` for mandatory requirements `missing` in the last review; it is not saved, and saving with `ai_interaction_id` confirms the AI interaction (`ai_assisted` in the audit)
- `needs_revaluation` for change management (M04)
- `na_justification` — free text saved by `evaluate_control()` when status=`na`; exposed in the detail API and used by the export engine (VDA and SOA)

**`calc_maturity_level` mapping:**

| Status | ML | Notes |
|--------|----|-------|
| `non_valutato` | 0 | — |
| `gap` | 1 | — |
| `na` | 0 | Excluded from average ML in VDA ISA |
| `parziale` | 2 | — |
| `compliant` | 3 / 4 | 4 if ≥ 2 valid evidences; 5 only via manual override (`set-maturity`) |
...
class ControlInstance(BaseModel):
    ...
    na_justification = models.TextField(blank=True)       # N/A justification — used by VDA and SOA
...
### `propagate_control()` service

Defined in `apps/controls/services.py`. Propagates the status of a `ControlInstance` to mapped controls, respecting relationship type and direction.
...
| Relationship (`ControlMapping.relationship`) | Propagation direction |
|----------------------------------------------|----------------------|
| `equivalente` | Bidirectional (A→B and B→A) |
| `covers` | Source→target only (A covers B; not vice versa) |
| `parziale`, `correlato`, `extends` | Not propagated |

- Only `compliant` and `na` statuses are propagable.
- Each updated instance generates an audit trail entry (`control.propagated`).
- For `na`: copies `na_justification` with prefix "Propagato da {external_id}".
- `cross_plant=True`: propagates to all plants with an active instance of the target control.
- Does not validate evidence requirements (data already validated on the source).

**REST endpoint:**
```
POST /api/controls/instances/{id}/propagate/
Body (optional): { "cross_plant": true }
...
- L1 (security), L2 (compliance), L3 (operational): event **classification** levels (retention reference: 5/3/1 years)
- **No deletion task**: the audit log is append-only/immutable. The PostgreSQL trigger rejects UPDATE and DELETE, so there is no retention-by-deletion (rule #4). Retention is effectively permanent; L1/L2/L3 are classification levels only.
...
# Run on demand or via cron (no scheduled beat task by default)
# Exits with a non-zero code if the chain is broken
```
...
def suggest_severity(incident, user) -> dict | None:
    try:
        result = route(
            task_type="incident_classify",   # local or cloud according to the routing in Settings → AI Engine
            prompt=f"...{incident.description}",
            user=user,
            entity_id=incident.pk,
            module_source="M09",
            sanitize=True,                   # mandatory with free text (rule #9)
            plant_ids=[incident.plant_id],
        )
    except (AiNotConfigured, LlmUnavailable):
        return None                          # AI not configured or unreachable: no suggestion
    # result = {"text", "provider", "model", "interaction_id", ...}
    # Offered to the user, never applied on its own: confirm_output() / ignore_output()
    return result
```
...
### Training (M15): no external integration

M15 does not integrate with e-learning or phishing-simulation platforms (the KnowBe4 integration has been removed): the platform **governs** training, it does not deliver it. Data are counts per group only: no employee register.

- **Model** (`apps/training/models.py`): `TrainingCourse` (type, audience, `validity_months`, `plants` = scope: empty = organisation course, otherwise those sites only; `controls` deprecated), `TrainingEvidenceControl` (single setting: per `audience_kind`, the `Control`s a session proves; initial values from `load_training_evidence_controls`, migration `training.0006` copies the old `controls` of active courses into it), `TrainingAudience` (per-site target group with `headcount`), `TrainingPlan` (per site or organisation-wide, with an M07 `document` for approval), `TrainingPlanItem`, `TrainingSession` (session with counts, `evidence` and `legacy` flag).
- **Services** (`apps/training/services.py`): `register_session` validates the file (`validate_uploaded_file`), creates the `Evidence` and links it to the site's `ControlInstance`s (`link_evidence_to_controls`); `remind_plan_items` opens tasks for the `compliance_officer` role (source_module `M15`); indicators shared by KPIs, Reporting and Cockpit (`training_coverage`, `plan_progress`, `latest_phishing`, `expiring_training_evidence`).
- Scope and linking: `create_course`/`update_course`/`delete_course` require an organisation-wide scope for courses without sites and management of all their sites for the others; `course_applies_to` restricts plan items (organisation plan → organisation courses only) and sessions to courses valid for the site. `link_evidence_to_controls` uses `evidence_controls_for(audience_kind)` filtered on `get_active_frameworks(plant)`: controls of frameworks not applied at the site are ignored, not reported. API `/api/v1/training/evidence-controls/` (GET/POST/DELETE; writing requires an organisation-wide scope).
- **Permissions**: write access for `super_admin`/`compliance_officer`/`plant_manager` within their scope or for an active CISO appointment in Governance (`can_manage_training`); read access for auditors too. `GET /api/v1/training/courses/capabilities/` tells the UI what to show.
...
| `check_training_migration_readiness` | Read-only preview of the training data migration (`training.0004`) | Before the `migrate` that applies it |
| `load_training_evidence_controls` | Adds the default controls proven by training sessions (ACN PR.AT-01/02, ISO A.6.3, NIS2 ART21-7.1, TISAX ISA-2.1.3), only if present; idempotent, removes nothing | After `load_frameworks`, on the first deploy of evidence-based training |
...
- Dedicated branches for large features; for targeted fixes and documentation, direct commits to `main` are allowed (repo convention)
...
1. In **Settings → AI Engine** check that an active configuration exists and that the function is routed to the right provider (routing per function)
