...
**PDCA existants, même clôturés** — un constat peut être lié à n'importe quel PDCA du même site, **même déjà clôturé ou archivé** : c'est le cas d'une action corrective réalisée avant l'enregistrement du constat (ex. un PDCA ouvert manuellement à partir du rapport d'audit) ou de la récupération de l'historique. Si le constat est ouvert et le PDCA clôturé, le constat passe **en réponse**, prêt à être clôturé avec une preuve ; si le constat est déjà clôturé, aucun statut ne change (lien a posteriori, enregistré comme tel dans la piste d'audit). Depuis le menu PDCA, **🔗 Constats** affiche tous les constats de l'audit, ouverts et clôturés.
