...
Le VDA ISA se remplit en anglais : pour chaque contrôle TISAX, l'auditeur lit, à côté de la maturité déclarée, **comment** l'exigence est mise en œuvre. L'onglet **Évaluation** du contrôle contient l'encadré « Implementation description (VDA ISA) » : vous pouvez écrire directement le texte en anglais ou utiliser l'entretien guidé, qui simule un échange avec un auditeur TISAX.

1. Cliquez sur **Remplir avec l'entretien guidé (IA)**
2. L'auditeur vous pose **2 à 4 questions par thème** dans votre langue (par exemple, pour la politique de sécurité : les documents, leur approbation et leur révision, la communication aux employés et aux partenaires). Pour chaque question, vous voyez **ce que l'auditeur veut comprendre**, **ce qu'il faut mentionner** et, sur demande, **un exemple** avec des espaces réservés entre crochets : adaptez-le à votre réalité, ne le copiez pas. « Exigences VDA de cette question » affiche les exigences originales en anglais (pour les contrôles L3 aussi celles du contrôle L2 étendu)
3. Répondez concrètement : quels documents, qui est responsable, comment cela se passe, à quelle fréquence. Écrivez « non » si quelque chose n'est pas encore en place. Les noms de personnes et les données personnelles ne sont pas nécessaires
4. **Vérification avec l'auditeur** (au maximum **2 tours**) : pour chaque exigence, l'auditeur indique si elle est couverte, partielle ou non couverte, la **maturité** que les réponses soutiennent (avec un avertissement si elle est inférieure à celle déclarée), les **preuves** qu'il demandera sur site (en signalant celles déjà liées au contrôle) et jusqu'à 3 **questions d'approfondissement** sur ce qui manque. Répondez aux approfondissements et, si nécessaire, faites le second tour. Une fois les tours utilisés, la vérification peut être recommencée : les réponses aux questions sont conservées
5. **Générer le brouillon en anglais** : l'IA rédige la description à partir de tout l'échange, par thème et sans répéter les faits, en n'utilisant que ce que vous avez écrit. Les exigences obligatoires encore non couvertes lors de la dernière vérification sont marquées `[TO BE COMPLETED]`. La traduction dans votre langue est affichée pour vérification
...
Les réponses s'enregistrent avec **Enregistrer les réponses** (la vérification et le brouillon les enregistrent aussi) et restent sur le contrôle pour la réévaluation de l'année suivante. L'entretien reste dans la langue dans laquelle il a été commencé. Le brouillon n'est jamais enregistré tout seul : la piste d'audit enregistre qui a enregistré la description et si elle provenait de l'IA. Si l'IA n'est pas configurée, l'entretien n'est pas disponible et l'encadré affiche les exigences originales pour rédiger la description manuellement. Le Centre opérationnel signale les contrôles TISAX de maturité ≥ 3 sans description.
...
3. Sélectionnez le type d'export :
   ...
   - **VDA ISA TISAX** — Tableau VDA Information Security Assessment, au format paysage (A4 paysage) : pour chaque contrôle maturité, statut, owner, **Implementation description**, **Reference documentation** (documents et preuves liés) et Note / Justification. Les contrôles de maturité ≥ 3 sans description sont marqués « Missing » et comptés dans l'en-tête. La description se renseigne dans l'onglet **Évaluation** du contrôle, encadré « Implementation description (VDA ISA) »
...
Les cycles d'organisation sont visibles par tous les sites, mais seuls les utilisateurs ayant accès à toute l'organisation peuvent les ouvrir et les faire avancer (avancement de phase, modification, archivage, suppression) ; pour les autres, ils apparaissent en **lecture seule**, avec le dossier consultable. En filtrant la liste par site, les cycles d'organisation apparaissent aussi, car ils s'y appliquent ; l'option **Organisation uniquement** n'affiche que ceux-ci. Le filtre **Statut** affiche les cycles en cours (de PLAN à ACT), une seule phase, les clôturés ou les archivés. Dans la phase DO d'un cycle d'organisation, on peut joindre une preuve de n'importe quel site. À la clôture, la leçon apprise générée est elle aussi d'organisation et, si le CHECK est ko, le nouveau cycle reste d'organisation.

**Responsable et date prévue** — dans le cycle (à la création ou avec ✏️) indiquez le **Responsable de l'action** et la **Date prévue** d'achèvement. Le responsable est un texte libre : il peut s'agir d'une personne qui n'utilise pas le portail (par ex. les RH d'un site ou un fournisseur) ; préférez la fonction au nom de la personne. Dans la liste, ils apparaissent sous le titre et un cycle non clôturé après la date est marqué **en retard** ; la revue de direction liste les cycles en retard avec leur responsable. Les tâches et notifications restent assignées aux rôles du portail. À partir de cette version, la liste n'a plus la colonne Périmètre : le site ou «Organisation» figure dans la colonne Site, le périmètre reste dans le dossier du cycle.
...
**PDCA existants, même clôturés** — un constat peut être lié à n'importe quel PDCA du même site, **même déjà clôturé ou archivé** : c'est le cas d'une action corrective réalisée avant l'enregistrement du constat (ex. un PDCA ouvert manuellement à partir du rapport d'audit) ou de la récupération de l'historique. Si le constat est ouvert et le PDCA clôturé, les règles de clôture du PDCA décrites ci-dessous s'appliquent (avec un résultat efficace, le constat est clôturé immédiatement) ; si le constat est déjà clôturé, aucun statut ne change (lien a posteriori, enregistré comme tel dans la piste d'audit). Depuis le menu PDCA, **🔗 Constats** affiche tous les constats de l'audit, ouverts et clôturés.

**Remplacer un PDCA** — si le constat a déjà un PDCA (pour les NC, celui ouvert automatiquement), on le remplace par le bon en indiquant un motif : depuis le constat avec **Remplacer le PDCA**, ou depuis le menu PDCA en choisissant le constat (il affiche le cycle auquel il est lié). Le PDCA remplacé n'est archivé que s'il avait été ouvert automatiquement pour ce constat et jamais travaillé ; sinon il reste tel quel, simplement détaché. Aucun nouveau PDCA n'est ouvert pour un constat clôturé : on le lie à un PDCA existant.
...
Si le constat a un PDCA lié, il ne se clôture qu'une fois l'action corrective terminée : PDCA **clôturé** (ou archivé), ou en phase **ACT** si le PDCA ne couvre que ce constat ; dans ce cas, le PDCA est clôturé avec le constat et les notes de clôture deviennent sa standardisation. Sinon, le système indique la phase du PDCA et n'enregistre rien. **Clôture depuis le PDCA** — le PDCA contient déjà la preuve (phase DO), la vérification d'efficacité (CHECK) et la standardisation (ACT) ; à sa clôture, les constats liés suivent le résultat du CHECK : **efficace** → ils sont clôturés avec lui, avec la preuve de la phase DO et la description ACT comme notes de clôture (une NC nécessite la preuve DO, sinon elle reste « en réponse ») ; **partiellement efficace** → ils passent « en réponse » et vous décidez de la clôture depuis l'onglet Constats ; **non efficace** → ils passent au nouveau cycle « [Riciclo] » et restent ouverts. Aucune leçon apprise supplémentaire n'est créée : celle du PDCA suffit. Si un constat est resté « en réponse » avec le PDCA déjà clôturé (par exemple lié avant cette règle, ou avec un résultat partiel que vous jugez suffisant), le bouton **Clôturer avec le PDCA** le clôture avec la preuve DO et la description ACT du PDCA, sans les ressaisir.
...
**Constat non poursuivi** — pour une observation ou une opportunité que vous décidez de ne pas mettre en œuvre, utilisez **Ne pas poursuivre** sur la carte du constat. Indiquez le motif (obligatoire, 20 caractères minimum) et, si vous le souhaitez, joignez une preuve : un fichier (par ex. le procès-verbal de la décision) ou une preuve existante. Le constat passe à **Non poursuivi** : il n'est plus compté comme ouvert, mais reste distinct des constats clôturés dans le paquet d'audit et dans la revue de direction, qui liste les constats écartés avec leur motif. Le PDCA lié encore ouvert est archivé avec le même motif ; pour un constat commun, la décision s'applique à tous les sites. Avec **Rouvrir**, le constat redevient ouvert et un nouveau PDCA est ouvert ou lié. Les non-conformités ne peuvent pas être non poursuivies : elles suivent le PDCA. Le même résultat s'obtient depuis le module PDCA avec 📦 **Archiver** : la fenêtre demande le motif et la preuve facultative, et les observations et opportunités liées encore ouvertes passent à «Non poursuivi» ; les non-conformités liées restent ouvertes et la fenêtre le signale. Pour un PDCA sans constat, la preuve reste sur le cycle et est visible dans son dossier.
...
**Ce qui est contrôlé sur les fournisseurs** — seulement ce qui peut faire d'un fournisseur une porte d'entrée chez vous, avec des sources publiques et passives (aucun scan de leurs systèmes) :
- **Compromission** : présence sur les sites de fuite ransomware (ransomware.live, mis à jour toutes les 6 heures), violations connues de leur service (HIBP), listes de malwares et listes noires.
- **Usurpation** : domaine usurpable (DMARC non bloquant : n'importe qui peut envoyer des e-mails « du fournisseur », le vecteur de la fraude aux factures), domaines sosies enregistrés dans l'année avec messagerie, domaine expiré ou bientôt expiré.
- **Exposition** (fournisseurs critiques uniquement) : accès distants (VPN, passerelles, webmail, SFTP…) trouvés dans les journaux de certificats et vérifiés avec Shodan InternetDB ; critique s'ils présentent une vulnérabilité du catalogue CISA KEV, c'est-à-dire activement exploitée.
- **Services utilisés** : dans la fiche fournisseur, onglet Posture externe, indiquez les adresses des services que vous utilisez réellement (portail de commandes, SFTP…) : seul leur certificat compte.

Les **fournisseurs critiques**, avec surveillance approfondie et scan hebdomadaire, sont ceux pertinents NIS2 ou TISAX, à risque élevé ou critique, ou mainteneurs de vos actifs OT. L'hygiène de façade (certificat du site vitrine, en-têtes, DNSSEC, MTA-STS) reste dans la dimension **Maturité**, qui pèse peu et ne génère aucun signalement. Chaque problème indique aussi **ce que vous devez faire** : par exemple, pour un domaine usurpable, demander à la comptabilité de vérifier par téléphone tout changement de coordonnées bancaires.
