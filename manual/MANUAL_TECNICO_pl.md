...
backend (Django + DRF)
    ...
    └── Celery Worker  zadania asynchroniczne: powiadomienia, zadania audit trail
        Celery Beat    harmonogram cykliczny: terminy i przypomnienia, nocna kopia zapasowa, KPI, OSINT
```
...
- `implementation_interview` — wywiad VDA ISA „w stylu audytora” (`services/vda_interview.py`, `VdaInterviewPermission`: tylko role oceniające kontrole): `GET|POST /vda-interview/`, `POST /vda-interview/review/`, `POST /vda-interview/draft/`. Wymagania wyodrębniane z opisu EN frameworku (sekcje must / should / wysoka / bardzo wysoka ochrona; dla VH także wymagania rozszerzanej kontroli bazowej), `req_id` = hash tekstu. AI grupuje je w 2-4 tematy (pytanie, intencja audytora, co wymienić, przykład z symbolami zastępczymi) w cache `Control.translations[lang]["interview_topics"]` powiązanym z zestawem wymagań (tekst normatywny, bez sanitize); przy pierwszym zapisie tematy i język stają się migawką w stanie. Weryfikacja audytora maks. 2 rundy (`MAX_REVIEW_ROUNDS`): pokrycie każdego wymagania, ≤ 3 pytania uzupełniające, dowody, potwierdzona dojrzałość; `reset_reviews` zaczyna ją od nowa. Szkic (zadanie AI `vda_interview`, Sanitizer) z całej rozmowy z `[TO BE COMPLETED]` dla obowiązkowych wymagań `missing` w ostatniej weryfikacji; nie jest zapisywany, a zapis z `ai_interaction_id` potwierdza interakcję AI (`ai_assisted` w audycie)
...
class ControlInstance(BaseModel):
    ...
    na_justification = models.TextField(blank=True)       # uzasadnienie N/A — używane przez VDA i SOA
...
### Serwis `propagate_control()`

Zdefiniowany w `apps/controls/services.py`. Propaguje status `ControlInstance` do zmapowanych kontroli z uwzględnieniem typu relacji i kierunku.

```python
propagate_control(instance, user, cross_plant=False) -> dict
# Zwraca: {"propagated_to": int, "skipped_no_instance": int}
```

**Reguły propagacji:**

| Relacja (`ControlMapping.relationship`) | Kierunek propagacji |
|-----------------------------------------|---------------------|
| `equivalente` | Dwukierunkowy (A→B i B→A) |
| `covers` | Tylko źródło→cel |
| `parziale`, `correlato`, `extends` | Nie propagowane |

- Propagowane są tylko statusy `compliant` i `na`.
- Każda zaktualizowana instancja generuje wpis audytu (`control.propagated`).
- Dla `na`: kopiuje `na_justification` z prefiksem "Propagato da {external_id}".
- `cross_plant=True`: propaguje do wszystkich plantów z aktywną instancją kontroli docelowej.

**Endpoint REST:**
```
POST /api/controls/instances/{id}/propagate/
Body (opcjonalnie): { "cross_plant": true }
Odpowiedź: { "propagated_to": 3, "skipped_no_instance": 0 }
```
...
| `reporting/compliance-overview/` | GET | Zgodność: podsumowanie według frameworku (%, statusy, trend) i porównanie zakłady × frameworki |
| `reporting/compliance-domains/` | GET | Zgodność: otwarte kontrole według domeny frameworku (`&export=csv` dla otwartych kontroli) |
...
```
apps/ai_engine/
├── sanitizer.py                            # anonimizacja danych osobowych przed chmurą
├── router.py                               # routing lokalny/chmura dla każdej funkcji, confirm/ignore
├── tasks_ai.py                             # funkcje AI: klasyfikacja incydentów, wyjaśnienie kontroli, działania dla luk, szkic RCA
├── agent_orchestrator.py + agent_tools.py  # asystent GRC: luki zakładu i wyjaśnienia
├── catalog.py                              # katalog modeli dostawców
├── circuit_breaker.py                      # wyłącznik (circuit breaker) dla dostawców
├── models.py                               # AiInteractionLog
├── tasks.py                                # retencja AiInteractionLog (miesięczne czyszczenie)
└── tests/
```
...
- **Model** (`apps/training/models.py`): `TrainingCourse` (rodzaj, odbiorcy, `validity_months`, `plants` = zakres: puste = kurs organizacji, w przeciwnym razie tylko te zakłady; `controls` przestarzałe), `TrainingEvidenceControl` (jedno ustawienie: dla `audience_kind` obiekty `Control` potwierdzane przez szkolenie; wartości początkowe z `load_training_evidence_controls`, migracja `training.0006` kopiuje do niego dawne `controls` aktywnych kursów), `TrainingAudience` (grupa docelowa zakładu z `headcount`), `TrainingPlan` (dla zakładu lub organizacji, z `document` M07 do zatwierdzenia), `TrainingPlanItem`, `TrainingSession` (szkolenie z liczbami, `evidence` i flagą `legacy`).
...
- Zakres i powiązania: `create_course`/`update_course`/`delete_course` wymagają zakresu całej organizacji dla kursów bez zakładów i zarządzania wszystkimi ich zakładami dla pozostałych; `course_applies_to` ogranicza pozycje planu (plan organizacji → tylko kursy organizacji) i szkolenia do kursów ważnych dla zakładu. `link_evidence_to_controls` używa `evidence_controls_for(audience_kind)` filtrowanego po `get_active_frameworks(plant)`: kontrole frameworków niestosowanych w zakładzie są pomijane, nie zgłaszane. API `/api/v1/training/evidence-controls/` (GET/POST/DELETE; zapis tylko z zakresem organizacji).
...
- **Role krytyczne i organ zarządzający**: dla kursów, których `audience_kind` nie jest `generale` (bez phishingu, `TrainingCourse.is_named`), szkolenie rejestruje `TrainingParticipant` (FK `user` i/lub `committee_member`, `roles` = migawka aktywnych nominacji). `participant_options(plant, day)` wymienia posiadaczy aktywnych `RoleAssignment` obejmujących zakład oraz urzędujących `CommitteeMember` organów zakładu lub organów bez zakładów; `register_session` przyjmuje `participant_users`/`participant_members`, waliduje je względem tych opcji i scala osoby będące jednocześnie członkami i posiadaczami nominacji. Jeśli kurs ma `competency`/`competency_level`, `_apply_competency` aktualizuje `UserCompetency` (dowód, `valid_until`, `verified_by`) bez obniżania poziomu i bez zastępowania nowszego dowodu, zapisując poprzedni stan w `competency_before`; po usunięciu szkolenia `_revert_competencies` go przywraca (cofając się przez już usunięte szkolenia) lub wykonuje soft delete kompetencji utworzonej przez szkolenie. `UserCompetency` ma teraz warunkowe ograniczenie unikalności (`uniq_user_competency_alive`, tylko nieusunięte wiersze). `board_training(plant)` zasila wewnętrzny KPI `board_training_valid` (urzędujący członkowie `SecurityCommittee` typu `cda` z ważnym szkoleniem z kursu `organo_gestione`), sekcję Reporting (tylko liczby), `board_training.csv` w pakiecie audytowym oraz `GET /api/v1/training/sessions/board-status/?plant=`. Inne endpointy: `GET sessions/participant-options/?plant=&held_on=` (tylko dla zarządzających szkoleniami zakładu) i `GET courses/competency-options/`. W audit trail tylko id i `participants` (liczba).
- **Dane historyczne**: zapisy i wyniki phishingu dla osób (`TrainingEnrollment`, `PhishingSimulation`) są agregowane przez migrację `training.0004` w sesje `legacy` zawierające wyłącznie liczby; migracja `training.0007`, w tym samym `migrate`, usuwa następnie tabele osobowe i przestarzałe pola kursu (`framework_refs`, `controls`).
...
| `load_training_evidence_controls` | Dodaje domyślne kontrole potwierdzane przez szkolenia (ACN PR.AT-01/02, ISO A.6.3, NIS2 ART21-7.1, TISAX ISA-2.1.3), tylko jeśli istnieją; idempotentne, niczego nie usuwa | Po `load_frameworks`, przy pierwszym wdrożeniu szkoleń opartych na dowodach |
...
| `DATABASE_URL` | string | postgresql://grc:REPLACE_DB_PASSWORD@db:5432/grc_dev | URL PostgreSQL | Tak |
...
- Dedykowane branche dla dużych funkcji; dla ukierunkowanych poprawek i dokumentacji dozwolone są commity bezpośrednio na `main` (konwencja repo)
