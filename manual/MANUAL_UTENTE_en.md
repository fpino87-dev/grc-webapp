...
Coverage (sample, extended, full) applies only to audits launched from an **annual programme**, where verification is spread over several quarters. An audit created on its own with **+ New preparation**, including a multi-site one, always covers **all controls** and shows no percentage.
...
**Existing PDCAs, including closed ones** — a finding can be linked to any PDCA of the same site, **even one already closed or archived**: this covers a corrective action carried out before the finding was recorded (e.g. a PDCA opened manually from the audit report) or history recovery. If the finding is open and the PDCA is closed, the PDCA closure rules described below apply (with an effective outcome the finding is closed immediately); if the finding is already closed no status changes (retroactive link, recorded as such in the audit trail). From the PDCA menu, **🔗 Findings** shows all the audit's findings, open and closed.
...
If the finding has a linked PDCA, it is closed only once the corrective action is complete: with the PDCA **closed** (or archived), or in the **ACT** phase if the PDCA covers only this finding; in that case the PDCA is closed together with the finding and the closure notes become its standardisation. Otherwise the system shows the PDCA phase and saves nothing. **Closing from the PDCA** — the PDCA already holds evidence (DO phase), effectiveness check (CHECK) and standardisation (ACT), so when you close it the linked findings follow the CHECK outcome: **effective** → they are closed with it, with the DO phase evidence and the ACT description as closure notes (an NC needs the DO evidence, otherwise it stays "in response"); **partially effective** → they move to "in response" and you decide the closure from the Findings tab; **not effective** → they move to the new "[Riciclo]" cycle and stay open. No extra lesson learned is created: the PDCA's one is enough. If a finding stayed "in response" with the PDCA already closed (for example linked before this rule, or with a partial outcome you consider sufficient), the **Close with the PDCA** button closes it using the PDCA's DO evidence and ACT description, without entering them again.
...
**Internal audit entrusted to an external consultant.** If the internal audit is conducted by a consultant, tick **Conducted by an external consultant** (when creating it or from the **Audit info** tab). The audit stays internal but is handled like a second-party audit: no checklist or readiness; record the consultant's findings and attach their report, which is included in the audit package (AUDIT_ESTERNI folder).

**Title** — edit the audit title with the pencil next to its name in the detail view. In a multi-site audit you edit the title of the common audit, which is copied to every site.
...
**Finding not pursued** — for an observation or opportunity you decide not to implement, use **Do not pursue** on the finding card. Write the reason (required, at least 20 characters) and, if you wish, attach a proof: a file (e.g. the minutes of the decision) or existing evidence. The finding becomes **Not pursued**: it no longer counts as open, but stays distinct from closed findings in the audit package and in the management review, which lists the discarded findings with their reason. The linked PDCA still open is archived with the same reason; for a common finding the decision applies to all sites. With **Reopen** the finding is open again and a new PDCA is opened or linked. Nonconformities cannot be left unpursued: they follow the PDCA. You get the same result from the PDCA module with 📦 **Archive**: the dialog asks for the reason and the optional proof, and the linked observations and opportunities still open become «Not pursued»; linked nonconformities stay open and the dialog says so. For a PDCA without findings the proof stays on the cycle and is visible in its dossier.
